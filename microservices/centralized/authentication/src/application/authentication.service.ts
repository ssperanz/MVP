...
export class AuthService {
  ...
  public async login(authentication: Authentication): Promise<string> {
    ...
    try {
      ...
      if (token) {
        if (token.getStatus() === TokenStatus.REVOKED) {
          await this.authRepository.updateToken(
            new Token(id as string, TokenStatus.ACTIVE),
          );
        } else {
      ...
      this.logger.log(
        `Generated JWT token for user ${authentication.getEmail()}: ${JWT}`,
      );
      ...
      this.logger.error(
        `Failed to login for user ${authentication.getEmail()}: ${error.message}`,
      );
      return Promise.resolve(
        JSON.stringify({
          error: {
  ...
  public async logout(sub: string): Promise<string> {
    ...
    if (token == null) {
      ...
      await this.authRepository.updateToken(
        new Token(sub, TokenStatus.REVOKED),
      );
    ...
    try {
      if (!jwt) {
        ...
        if (decoded) {
          ...
          if (
            listToken !== null &&
            listToken?.getStatus() === TokenStatus.ACTIVE
          ) {
            this.logger.debug(
              `JWT verified successfully: ${JSON.stringify(decoded)}`,
            );

            const token = { token: decoded };

            // Call the emit function to notify RESGATE of the token
            await this.outboundPortsAdapter.emitAccessToken(
            ...
            await this.outboundPortsAdapter.emitAccessToken(
              JSON.stringify({ token: { error: 'Token has been logged out' } }),
              cid,
            );
            return Promise.resolve(
              JSON.stringify({
          ...
          await this.authRepository.updateToken(
            new Token(
              (this.jwtService.decode(jwt) as { sub: string }).sub,
              TokenStatus.REVOKED,
            ),
          );
          return Promise.resolve(
      ...
      this.logger.error(
        `JWT verification failed: ${error.message}, with ${(this.jwtService.decode(jwt) as { sub: string }).sub}`,
      );
      await this.outboundPortsAdapter.emitAccessToken(
  ...
  public async registerGlobalSupervisor(
    globalSupervisor: GlobalSupervisor,
  ): Promise<string> {
    try {
      if (!(await this.isGlobalSet())) {
        const newGlobalId =
          await this.authRepository.newProfile(globalSupervisor);
        await this.authRepository.storeToken(
          new Token(newGlobalId, TokenStatus.REVOKED),
        );
        return Promise.resolve(
          JSON.stringify({
            result:
              'Global Supervisor registered successfully, with id: ' +
              newGlobalId,
          }),
        );
      } else {
  ...
  public async registerLocalSupervisor(
    LocalSupervisor: LocalSupervisor,
  ): Promise<string> {
    try {
      const newLocalId = await this.authRepository.newProfile(LocalSupervisor);
      await this.authRepository.storeToken(
        new Token(newLocalId, TokenStatus.REVOKED),
      );
      return Promise.resolve(
        JSON.stringify({
          result:
            'Local Supervisor registered successfully, with id: ' + newLocalId,
        }),
      );
    } catch (error) {
  ...
  private async generateJWT(user: User): Promise<string> {
    ...
    if (user.getRole() == Role.GLOBAL) {
      payload = {
        sub: await this.authRepository.getIdByEmail(
          user.getAuthentication().getEmail(),
        ),
        isGlobal: true,
      };
