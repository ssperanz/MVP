...
@QueryHandler(GetProductByWhIdQuery)
export class GetProductByWhIdQueryHandler implements IQueryHandler<GetProductByWhIdQuery> {
  constructor(
    @Inject('IProductReadModelRepository')
    private readonly productReadModel: ProductReadModelRepository,
  ) {}

  async execute(query: GetProductByWhIdQuery): Promise<ProductDto[] | null> {
    const productReadModels = await this.productReadModel.findByWhId(query.whId);
