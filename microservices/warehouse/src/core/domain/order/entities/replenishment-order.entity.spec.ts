import { ReplenishmentOrder } from './replenishment-order.entity.js';
...
describe('ReplenishmentOrder', () => {
  it('should create a ReplenishmentOrder and apply OrderCreatedEvent', () => {
    ...
    const orderType = OrderType.REPLENISHMENT;
    const departureWh = new WarehouseId(1);
    const destinationWh = new WarehouseId(2);
    const orderReference = new OrderId('order-ref-456');

    const replenishmentOrder = ReplenishmentOrder.create(orderId, orderItems, orderType, departureWh, destinationWh, orderReference);

    expect(replenishmentOrder).toBeInstanceOf(ReplenishmentOrder);
    expect(replenishmentOrder.getOrderReference()).toEqual(orderReference);

    const events = replenishmentOrder.getUncommittedEvents();
