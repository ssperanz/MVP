...
export class ValidateOrderCommandHandler
  implements ICommandHandler<ValidateOrderCommand>
{
  ...
  async execute(command: ValidateOrderCommand): Promise<void> {
    ...
    const trackedReservation =
      this.publisher.mergeObjectContext(reservation);

    const missingItems = trackedReservation.getMissingItems();

    if (missingItems.length > 0) {
      trackedReservation.pause();
      ...
      this.eventBus.publish(
        new OrderValidatedEvent(command.orderId),
      );
    }
  }
}