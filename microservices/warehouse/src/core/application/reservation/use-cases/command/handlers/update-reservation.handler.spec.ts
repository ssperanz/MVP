...
import { UpdateReservationCommandHandler } from './update-reservation.handler.js';

describe('UpdateReservationCommandHandler', () => {
  let commandHandler: UpdateReservationCommandHandler;
  ...
  beforeEach(() => {
    ...
    commandHandler = new UpdateReservationCommandHandler(reservationRepository, eventPublisher);
  });

  it('should update an existing reservation and commit the event', async () => {
    const command = new UpdateReservationCommand(new OrderId('order-1'), [/* some items */]);
    ...
    expect(mockReservation.reserve).toHaveBeenCalledWith(command.items);
